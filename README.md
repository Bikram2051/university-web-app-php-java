...
|---|---|---|
| `main` | PHP/MySQL source code | Read the full back-end implementation |
| `static-portfolio` | Static HTML conversion | Live demo hosted on Cloudflare Pages |

Recruiters viewing the repo land on `main` to see the full-stack code. The live demo link below points to the static conversion on `static-portfolio`.

---

## Live demo

> **[https://your-subdomain.pages.dev](https://your-subdomain.pages.dev)**
> *(replace with your Cloudflare Pages URL after deployment)*

The live demo is a static HTML/CSS/JS conversion of the original PHP project. Form submissions show a confirmation message rather than writing to a database. Backend-only pages (manager dashboard, order handlers) display a notice linking back to this repo.

---

## Tech stack

### Original PHP/MySQL version (`main`)
...
- **No build tools or frameworks** — deployed directly as static files
...
## Deployment (static version)

The `static-portfolio` branch is deployed to Cloudflare Pages with no build step:
